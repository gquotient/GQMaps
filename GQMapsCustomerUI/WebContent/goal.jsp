...
<head>
	...
	function goalSubmit(actionName) {
	  ...
	  alert ( 'tt form open on submit = ' );
	  xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
	  alert ( 'tt form open3 on submit = ' );

	  // send the collected data as JSON
	  xhr.send(formJson);
	  alert ( 'gspform before submit = ' + formJson );
      return true;
	}
	
///------------------

form.onsubmit = function (e , fdata) {
  // stop the regular form submission
  e.preventDefault(); 
  alert ( 'tt form mock on submit = ' );
...
<form name="ttform" action="/GQMapsCustomerServices/saveandFinalize/saveFinalize" method="POST">
